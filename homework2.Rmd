...
# Mutates are computationally inefficient.. you want to summarise to get totals first 
# and then mutate to get %. How about the following code

card_fraud %>%
  group_by(category, is_fraud) %>%
  summarise(total_txns = n(),
         total_amounts = sum(amt)) %>% 
  mutate(percent_total_txns =  total_txns/sum(total_txns),
         percent_amounts = total_amounts/sum(total_amounts)) %>% 
  ungroup() %>% 

# assuming you want to show % of amounts
